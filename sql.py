    ...
    def __init__(self, url, **kwargs):
        ...
        if matches:
            if not os.path.exists(matches.group(1)):
                raise RuntimeError(f"does not exist: {matches.group(1)}")
            if not os.path.isfile(matches.group(1)):
                raise RuntimeError(f"not a file: {matches.group(1)}")
    ...
    def execute(self, sql, *args, **kwargs):
        ...
        if paramstyle == "qmark":
            ...
            if len(placeholders) != len(args):
                if len(placeholders) < len(args):
                    raise RuntimeError(
                        f"fewer placeholders ({_placeholders}) than values ({_args})"
                    )
                else:
        ...
        # numeric
            ...
            for index, i in placeholders.items():
                if i >= len(args):
                    raise RuntimeError(f"missing value for placeholder (:{i + 1})")
                tokens[index] = self._escape(args[i])
        ...
        # named
            ...
            for index, name in placeholders.items():
                if name not in kwargs:
                    raise RuntimeError(f"missing value for placeholder (:{name})")
        ...
        # pyformat
            ...
            for index, name in placeholders.items():
                if name not in kwargs:
                    raise RuntimeError(f"missing value for placeholder (%{name}s)")
                tokens[index] = self._escape(kwargs[name])
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                _statement = "".join(
                    [
                        str(bytes) if token.ttype == sqlparse.tokens.Other else str(token)
                        for token in tokens
                    ]
                )
